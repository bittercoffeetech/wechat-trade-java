...
public enum ErrorCodeEnum {
	...
	private static final Map<ErrorCodeEnum, String> MESSAGES = ImmutableMap.<ErrorCodeEnum, String>builder()
			.put(ErrorCodeEnum.SYSTEMERROR, "系统错误").put(ErrorCodeEnum.XML_FORMAT_ERROR, "XML格式错误")
			.put(ErrorCodeEnum.APPID_NOT_EXIST, "APPID不存在").put(ErrorCodeEnum.MCHID_NOT_EXIST, "MCHID不存在")
			.put(ErrorCodeEnum.APPID_MCHID_NOT_MATCH, "appid和mch_id不匹配").put(ErrorCodeEnum.SIGNERROR, "签名错误")
			.put(ErrorCodeEnum.REQUIRE_POST_METHOD, "请使用post方法").put(ErrorCodeEnum.NOAUTH, "商户无此接口权限")
			.put(ErrorCodeEnum.INVALID_REQUEST, "参数错误").put(ErrorCodeEnum.LACK_PARAMS, "缺少参数")
			.put(ErrorCodeEnum.POST_DATA_EMPTY, "post数据为空").put(ErrorCodeEnum.NOT_UTF8, "编码格式错误")
			.put(ErrorCodeEnum.NOTENOUGH, "余额不足").put(ErrorCodeEnum.ORDERPAID, "商户订单已支付")
			.put(ErrorCodeEnum.ORDERCLOSED, "订单已关闭").put(ErrorCodeEnum.OUT_TRADE_NO_USED, "商户订单号重复")
			.put(ErrorCodeEnum.PARAM_ERROR, "参数错误").put(ErrorCodeEnum.INVALID_REQ_TOO_MUCH, "无效请求过多")
			.put(ErrorCodeEnum.FREQUENCY_LIMITED, "频率限制").put(ErrorCodeEnum.CERT_ERROR, "证书校验错误")
			.put(ErrorCodeEnum.INVALID_TRANSACTIONID, "无效transaction_id")
			.put(ErrorCodeEnum.BIZERR_NEED_RETRY, "退款业务流程错误，需要商户触发重试来解决").put(ErrorCodeEnum.TRADE_OVERDUE, "订单已经超过退款期限")
			.put(ErrorCodeEnum.ERROR, "业务错误").put(ErrorCodeEnum.USER_ACCOUNT_ABNORMAL, "退款请求失败")
			.put(ErrorCodeEnum.REFUND_FEE_MISMATCH, "订单金额或退款金额与之前请求不一致，请核实后再试")
			.put(ErrorCodeEnum.ORDER_NOT_READY, "订单处理中，暂时无法退款，请稍后再试").put(ErrorCodeEnum.REFUNDNOTEXIST, "退款订单查询失败")
			.build();
